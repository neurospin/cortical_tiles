...
# Default logging level of cortical_tiles
...
log_cortical_tiles = logging.getLogger('')
for hdlr in log_cortical_tiles.handlers[:]:
...
def set_file_logger(path_file):
    ...
    """
    global log_cortical_tiles
    return log_cortical_tiles.getChild(basename(path_file))
...
def set_file_log_handler(file_dir, suffix):
    ...
    for hdlr in log_cortical_tiles.handlers[:]:
        if isinstance(hdlr, logging.FileHandler):
            log_cortical_tiles.removeHandler(hdlr)
    log_cortical_tiles.addHandler(filehandler)
    for hdlr in log_cortical_tiles.handlers[:]:
        hdlr.setFormatter(formatter)

    # Logs name of log file
    simple_critical_log(log=log_cortical_tiles,
                        log_message=f"\nLog written to:\n{file_name}\n")
...
def simple_critical_log(log, log_message):
    ...
    for hdlr, form in zip(log_cortical_tiles.handlers[:], old_format):
